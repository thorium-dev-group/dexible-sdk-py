...
class TokenHelper:
	...
	async def find(self, provider, chain_id, address, owner=None):
		...
		try:
			info = await self.get_info(provider, chain_id, address, owner=owner)
	...
	async def get_info(self, provider, chain_id, address, owner=None):
		...
		mc_adddress = CHAIN_CONFIG[chain_id]["Multicall"]  # Multicall contract address
		settlement_address = CHAIN_CONFIG[chain_id]["Settlement"]  # Settlement contract address
		multicall = w3.eth.contract(abi=MULTICALL_ABI, address=w3.toChecksumAddress(mc_adddress))  # Multicall contract on chain
		...
		returndict = {}
		for call in calls:
			fn_abi = web3._utils.contracts.find_matching_fn_abi(call["abi"], w3.codec, call["method"], call["args"])
...
class TokenSupport:
    ...
    async def increase_spending(self, token, amount):
        ...
        time.sleep(30)  # Wait for 30s to allow network to propagate, as with original sdk
        token.allowance = amount
        self.tokenhelper.invalidate_cache_for(token.address)
        return tx_id

    async def verify(self, address):
        return await self.api_client.get(f"token/verify/{self.chain_id}/{address}")
