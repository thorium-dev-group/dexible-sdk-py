...
class Contact:
    ...
    async def toggle(self, id):
        return await self.api_client.post(f"contact-method/toggle/{id}", data={"id": id})
...
class Price:
    ...
    def inverse(self):
        return Price(self.out_token, self.in_token, self.out_amount, self.in_amount)

    def to_fixed(self, digits):
        return round(self.rate, digits)

    def to_string(self):
        return round(self.rate, min(self.in_token.decimals, self.out_token.decimals))
